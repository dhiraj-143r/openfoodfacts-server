...
document.addEventListener('DOMContentLoaded', function () {
    if (document.body.classList.contains('product_page')) {
        // Nutri-Score
        const nsImg = document.querySelector('img[src*="nutriscore-"]');
        if (nsImg) {
            const nsMatch = nsImg.src.match(/nutriscore-([a-e])/);
            if (nsMatch) {
        ...
        // Eco-Score
        const esImg = document.querySelector('img[src*="ecoscore-"]');
        if (esImg) {
            const esMatch = esImg.src.match(/ecoscore-([a-e])/);
            if (esMatch) {
        ...
        // NOVA group
        const novaImg = document.querySelector('img[src*="nova-group-"]');
        if (novaImg) {
            const novaMatch = novaImg.src.match(/nova-group-([1-4])/);
            if (novaMatch) {
